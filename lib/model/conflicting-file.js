...
export default class ConflictingFile {
  ...
  /*
   * Return a representation of this file's relative path that's safe to use as a CSS class.
   */
  safePath () {
    return this.path.replace(/[^a-z0-9]/g, (s) => {
      const code = s.charCodeAt(0)
      if (code >= 65 && code <= 90) return `__${s.toLowerCase()}`
      return '_' + ('000' + code.toString(16)).slice(-4)
    })
  }
