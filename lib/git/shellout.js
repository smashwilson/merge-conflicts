...
let BufferedProcess = atom.BufferedProcess;
let Directory = atom.Directory;
let fs = require("fs");
...
let runProcess = function (args) {
  return new BufferedProcess(args);
};

exports.mockProcess = function (runHandler) {
  runProcess = runHandler;
};
...
exports.getGitContext = function (filePath) {
  return Promise.all([locateGit(), common.getActiveRepo(filePath)])
    .then((results) => {
      ...
      return new GitContext(atomRepo, wd);
    })
};
